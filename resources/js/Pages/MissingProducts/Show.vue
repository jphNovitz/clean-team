...
function showComponent(typeID, productID) {
    if (typeID === 2) {
        if (products.filter(product => product.id == productID)[0].type_id === 2) return showConsumable.value
        else return true
    }
}

watch(showConsumable, async () => { showOddColor.value = showConsumable.value})
                    ...
                    <template #content>
                        <div class="flex justify-start my-4">
                            <label for="default-toggle" class="inline-flex relative items-center cursor-pointer">
                                <input type="checkbox" value="" id="default-toggle" v-model="showConsumable"
                                    class="sr-only peer">
                                <div
                        ...
                        <div v-for="line in journal" :key="line.id" class="flex items-center justify-between transition-colors ease-in-out delay-150 duration-1000"
                            :class="{'odd:bg-indigo-50' : showOddColor}">
                            <ProductLine :line="line" v-if="showComponent(2, line.product_id)"></ProductLine>
                        </div>
                    </template>
